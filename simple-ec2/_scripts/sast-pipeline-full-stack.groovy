    ...
    stages {
        stage('Prepare') {
            steps {
                ...
                script {
                    ...
                    echo "-> Total YAML files: ${totalYaml}"
                    echo "-> Total Dockerfiles: ${totalDockerfile}"
                    echo "-> Total Python files: ${totalPython}"
                    echo "-> Total Golang files: ${totalGolang}"
                }
            }
        }
        ...
        stage('Install SAST Tools') {
            steps {
                ...
                script {
                    def PYTHON_DEPENDENCIES = sh(script: 'find $WORKSPACE -type d -name "site-packages"', returnStdout: true).trim()
                    SAST_ENV = [
                    ...
                    withEnv(SAST_ENV) {
                        if ( totalYaml > 0 || totalDockerfile > 0) {
                            ...
                            def statusKicsScan = sh(
                                script: "kics scan --ci --no-color -p ${DIR_SOURCE} --output-path ${DIR_RESULTS} --report-formats 'json,html'", 
                                returnStatus: true, 
                                returnStdout: false)
                ...
                script {
                    withEnv(SAST_ENV) {
                        if ( totalYaml > 0 ) {
                            ...
                            def exitCodeYamlLint = sh(script: "yamllint ${DIR_SOURCE} -f parsable &> ${DIR_RESULTS}/yamllint-stdout.log", returnStatus: true)
                            echo "-> Yaml Linter exit code: ${exitCodeYamlLint}"
                            if ( exitCodeYamlLint > 0 ) {
                                echo "-> Publishing Yaml Linter reports."
                                recordIssues(
                                    enableForFailure: false,
                ...
                script {
                    withEnv(SAST_ENV) {
                        if ( totalPython > 0 ) {
                            ...
                            def exitCodePythonLint = sh(script: "pylint ${DIR_SOURCE} -f parseable -d I,R,C > ${DIR_RESULTS}/pylint-stdout.log", returnStatus: true)
                ...
                script {
                    withEnv(SAST_ENV) {
                        if ( totalGolang > 0 ) {
                            ...
                            echo "-> Golang Linter exit code: ${exitCodeGolangLint}"
